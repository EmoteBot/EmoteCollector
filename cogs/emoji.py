...
class Emotes:
	...
	async def steal_all(self, context, *args):
		"""Steal all emotes listed on a markdown file given by the first arg, or all emotes if len(args) > 1
		...
		if len(args) == 1:
			if not await self.bot.is_owner(context.author):
				return
			try:
				emotes = self.scrape_list(args[0])
			except asyncio.TimeoutError:
			...
			for match in self.RE_CUSTOM_EMOTE.finditer(''.join(args)):
				...
				emotes.append((name, image_url, context.author.id))
