...
class Emotes:
	...
	async def steal_all(self, context, list_url):
		...
		"""
		async with self.session.get(list_url, read_timeout=30) as resp:
	...
	async def add_backend(self, name, url, author_id):
		...
		async with self.session.head(url, read_timeout=5) as response:
			if response.reason != 'OK':
				raise HTTPException(response.status)
			if response.headers.get('Content-Type') not in ('image/png', 'image/jpeg', 'image/gif'):
				raise InvalidImageError

		async with self.session.get(url, read_timeout=30) as response:
			if response.reason != 'OK':
				raise HTTPException(response.status)
